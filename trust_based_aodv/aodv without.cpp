	...
	for(int i=0;i<max;i++)
	{
		...
		if(nodep[i].rreqqueuep.size==-1)
		{
			...
			for(int j=0;j<=nodep[i].rreqqueuep.size;j++)
			{
				...
				int k=0;
				g=1;
				//if(nodep[i].ip!=nodep[i].rreqqueuep.queuepointer[j].destip)
				//{		
...
int processrrep()
{
	for(int i=0;i<max;i++)
	{
		if(nodep[i].rrepqueuep.size==-1)
		{
			...
			{
				//if(nodep[i].ip==nodep[i].rrepqueuep.queuepointer[j].destip)
					//continue;
				int g=1;
